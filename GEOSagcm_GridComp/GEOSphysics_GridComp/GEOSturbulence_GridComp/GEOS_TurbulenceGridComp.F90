...
!    VERIFY_(STATUS)
    ...
    call MAPL_AddExportSpec(GC,                                &
       SHORT_NAME = 'HL2T_T',                                    &
       LONG_NAME  = 'turbulent_transport_of_liquid_water_static_energy_variance', &
       UNITS      = 'K+2s-1',                                    &
       DIMS       = MAPL_DimsHorzVert,                           &
       VLOCATION  = MAPL_VLocationEdge,               RC=STATUS  )
    VERIFY_(STATUS)
    ...
    call MAPL_AddExportSpec(GC,                                &
       SHORT_NAME = 'HLQTT_T',                                   &
       LONG_NAME  = 'turbulent_transport_of_liquid_water_static_energy-total_water_specific_humidity_covariance', &
       UNITS      = 'Ks-1',                                      &
       DIMS       = MAPL_DimsHorzVert,                           &
       VLOCATION  = MAPL_VLocationEdge,               RC=STATUS  )
    VERIFY_(STATUS)
...
!
    ...
    if ( DO_MYNN /= 0 ) then
       ...
       call MAPL_AddInternalSpec(GC,                                            &
            LONG_NAME  = 'surface_value_for_tke',                                 &
            UNITS      = 'm+2 s-2',                                               &
            DEFAULT    = 0.0,                                                     &
            FRIENDLYTO = 'TURBULENCE',                                            &
            SHORT_NAME = 'TKE_SURF',                                              &
       ...
       call MAPL_AddInternalSpec(GC,                                            &
            LONG_NAME  = 'surface_value_for_hl2',                                 &
            UNITS      = 'K+2',                                                   &
            DEFAULT    = 0.0,                                                     &
            FRIENDLYTO = 'TURBULENCE',                                            &
            SHORT_NAME = 'HL2_SURF',                                              &
       ...
       call MAPL_AddInternalSpec(GC,                                            &
            LONG_NAME  = 'surface_value_for_qt2',                                 &
            UNITS      = 'kg+2 kg-2',                                             &
            DEFAULT    = 0.0,                                                     &
            FRIENDLYTO = 'TURBULENCE',                                            &
            SHORT_NAME = 'QT2_SURF',                                              &
       ...
       call MAPL_AddInternalSpec(GC,                                            &
            LONG_NAME  = 'surface_value_for_hlqt',                                &
            UNITS      = 'K+1 kg+1 kg-1',                                         &
            DEFAULT    = 0.0,                                                     &
            FRIENDLYTO = 'TURBULENCE',                                            &
            SHORT_NAME = 'HLQT_SURF',                                             &
            DIMS       = MAPL_DimsHorzOnly,                                       &
            VLOCATION  = MAPL_VLocationNone,                                      &
            RC=STATUS  )
       VERIFY_(STATUS)
...
!---------------------------------
    ...
    ! To decide whether to include MYNN-related internal variables
    call MAPL_GetResource (MAPL, DO_MYNN,    "TURBULENCE_DO_MYNN:",    default=0, RC=STATUS)
    call MAPL_GetResource (MAPL, MYNN_LEVEL, "TURBULENCE_MYNN_LEVEL:", default=2,  RC=STATUS)
...
!----------------------
    if ( DO_MYNN /= 0 ) then
       ...
       VERIFY_(STATUS)
       call MAPL_GetPointer(INTERNAL, HL2_SURF,  'HL2_SURF',  RC=STATUS)
       VERIFY_(STATUS)
       call MAPL_GetPointer(INTERNAL, QT2_SURF,  'QT2_SURF',  RC=STATUS)
       VERIFY_(STATUS)
       call MAPL_GetPointer(INTERNAL, HLQT_SURF, 'HLQT_SURF', RC=STATUS)
       VERIFY_(STATUS)
    end if
...
! EDMF variables
   ...
   real, dimension(IM,JM,0:LM)          ::  ae3,aw3,aws3,awqv3,awql3,awqi3,awu3,awv3,awtke3
...
!========== SHOC ===========
     ...
     VERIFY_(STATUS)
     call MAPL_GetPointer(EXPORT,        wu,        'wu', ALLOC=.TRUE., RC=STATUS)
     VERIFY_(STATUS)
...
!             U,V,THL,THV,QT,Q,QL,QI,USTAR,SH,EVAP,zpbl_test,ice_ramp, &
             U,V,THL,THV,QT,Q,QL,QI,TKESHOC,USTAR,SH,EVAP,zpbl,ice_ramp, &
             ...
             edmfdryv,edmfmoistv,  &
             edmfmoistqc,             &
             ae3, aw3, aws3, awqv3, awql3, awqi3, awu3, awv3, awtke3, &
...
#endif
             EDMF_DISCRETE_TYPE, EDMF_IMPLICIT, DOCLASP)
...
!----------------------------------------------
      ...
      call write_parallel( "Doing LU decomp..." )
...
!----------------------------------------
    ...
    call write_parallel( "Looping over diffuse bundle..." )
...
!          else
!             SX_HALF = sign( min( abs(SX_HALF), sqrt(HL2*QT2) ), SX_HALF )
          end if
       else
...
!------------------
       ...
       if ( associated(SOI) ) then
          if ( trim(name) == 'TKE_NEW' .and. associated(TKET_T) .and. associated(TKET_D) ) then
             TKET_T(:,:,1:LM-1) = SOI(:,:,1:LM-1) - TKET_M(:,:,1:LM-1) - TKET_B(:,:,1:LM-1) - TKET_D(:,:,1:LM-1)
...
! -----------------------------------------------------
    ...
    call write_parallel( "  diffusion loop complete!" )
...
SUBROUTINE EDMF(its,ite,kts,kte,dt,zlo3,zw3,pw3,rhoe3,nup,&
              u3,v3,thl3,thv3,qt3,qv3,ql3,qi3,tke3,&
...
! s_aw3,s_awthl3 ... (ITS:ITE,KTS-1:KTE)
       ...
       REAL,DIMENSION(ITS:ITE,KTS:KTE), INTENT(IN) :: U3,V3,THL3,QT3,THV3,QV3,QL3,QI3,TKE3,ZLO3
...
! outputs
  ...
  ! outputs - variables needed for solver (s_aw - sum ai*wi, s_awphi - sum ai*wi*phii)
        REAL,DIMENSION(ITS:ITE,KTS-1:KTE), INTENT(OUT) :: ae3,aw3,aws3,awqv3,awql3,awqi3,awu3,awv3,awtke3
   ...
   ! output - buoyancy flux: sum_i a_i*w_i*(thv_i-<thv>) ... for TKE equation
         REAL,DIMENSION(ITS:ITE,KTS:KTE), INTENT(OUT) :: buoyf,mfw2,mfw3,mfqt3,mfqt2,mfwqt,mfhl2,&
                                                         mfhlqt,mfwhl, hle, qte, E, D     
! updraft properties
...
! internal surface cont
      REAL :: UST,WTHL,WQT,PBLH
       REAL,DIMENSION(KTS-1:KTE) :: dry_a, moist_a,dry_w,moist_w, &
        ...
        REAL,DIMENSION(KTS-1:KTE) :: s_aw2,s_aw3,s_aqt3,s_aqt2,s_ahlqt,s_awqt,s_ahl2,s_awhl
        REAL,DIMENSION(KTS-1:KTE) :: s_awthv ! for MYNN
! exner function
        REAL,DIMENSION(KTS:KTE) :: exf 
        REAL,DIMENSION(KTS-1:KTE) :: exfh

        real                      :: au_full, thlu_full, qtu_full
        REAL,DIMENSION(KTS-1:KTE) :: rhoe, au_flip, wu_flip, Mu_flip, thlu, qtu
        REAL,DIMENSION(KTS:KTE)   :: E_flip, D_flip, hle_flip, qte_flip
!  
...
! constants get from resources
 real :: ENT0,pwmin,pwmax,AlphaW,AlphaQT,AlphaTH,EDfac,mftot
...
if (L0(IH) .gt. 0. ) then
   ...
   do i=1,Nup2   
    do k=kts,kte
!     ENT(k,i)=real(ENTi(k,i))*Ent0/(ZW(k)-ZW(k-1)) ! stochastic
     ENT(k,i)=ENTf(k,i)*Ent0/(ZW(k)-ZW(k-1))        ! fixed
   enddo
   enddo
...
!                 UPW(K,I)=sqrt(Wn2)   
  ...
  !     
      ...
      DO k=KTS-1,KTE  ! loop in vertical 
        ...
        thl_plume10(IH,k)= upthl(kte+kts-k-1,10)
         ...
         DO I=1,NUP2 ! first sum over all i-updrafts
            ...
            if ( k >= KTS ) then
               E_flip(k) = E_flip(k) + rhoe(k-1)*UPA(K-1,I)*UPW(K-1,I)*Ent(K,I)
            end if
         ...
         ! Save updraft conserved properties
         au_flip(k) = dry_a(k) + moist_a(k)
         if ( au_flip(k) > 0. ) then            
            if ( moist_a(k) > 0. ) then
               wu_flip(k) = ( dry_a(k)*dry_w(k)   + moist_a(k)*moist_w(k)  )/au_flip(k)
               thlu(k)    = ( dry_a(k)*dry_thl(k) + moist_a(k)*moist_thl(k) )/au_flip(k)
               qtu(k)     = ( dry_a(k)*dry_qt(k)  + moist_a(k)*moist_qt(k)  )/au_flip(k)
            else
               wu_flip(k) = dry_w(k)
               thlu(k)    = dry_thl(k)
               qtu(k)     = dry_qt(k)
            ...
            Mu_flip(k) = rhoe(k)*au_flip(k)*wu_flip(k)
         else
            wu_flip(k) = 0.
            thlu(k)    = thli(k)
            qtu(k)     = qti(k)
   ...
   do k = kts,kte
      !
      D_flip(k) = E_flip(k) - ( Mu_flip(k) - Mu_flip(k-1) )/( zw(k) - zw(k-1) )

      !
      au_full   = 0.5*( au_flip(k-1) + au_flip(k) )
      thlu_full = 0.5*( thlu(k-1)    + thlu(k) )
      qtu_full  = 0.5*( qtu(k-1)     + qtu(k) )

      if ( au_full > 0. ) then
         hle_flip(k) =   exf(k)*( thl(k) - au_full*thlu_full )/( 1. - au_full ) &
                       + (mapl_grav/mapl_cp)*zlo(k)
         qte_flip(k) = ( qt(k)  - au_full*qtu_full  )/( 1. - au_full )
      else
         hle_flip(k) = exf(k)*thl(k) + (mapl_grav/mapl_cp)*zlo(k)
         qte_flip(k) = qt(k)
      end if
   end do
...
!      awtke3(IH,K)=s_awtke(KTE+KTS-K-1)
